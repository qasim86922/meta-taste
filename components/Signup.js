...
const Signup = ({ navigation }) => {
  ...
  const onPressSignup = async () => {
    ...
    if (inputs && Object.keys(inputs).length !== 14)
      setError("You must fill all enteries");
    else if (inputs && inputs.password !== inputs.confirmPassword)
    ...
    <ImageBackground source={image} style={styles.image}>
      <View style={styles.signup}>
        ...
        <TextInput
          ...
          height="10%"
          width="90%"
          onChangeText={(text) => setInputs({ ...inputs, otherDisease: text })}
        />
